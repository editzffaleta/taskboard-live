...
Contratos de destaque na base: `Entity` (com `equals`, `clone`, `cloneProps` + diff tipado e `toJSON`), `UseCase`, repositorios CRUD, `TransactionManager` (abstrai transacao sem expor o ORM; os repositorios de escrita — create/update/delete — aceitam `tx?: TransactionContext`), erros de dominio e a biblioteca de validacao (`Validator` + regras).
