...
function main() {
  ...
  const originalTemplateName = targetPackageJson.name;
  if (originalTemplateName && originalTemplateName !== TEMP_PACKAGE_NAME) {
    console.log(
      `Aviso: template package.json usava "${originalTemplateName}" em vez do placeholder "${TEMP_PACKAGE_NAME}". Substituindo pelo scope detectado.`,
    );
  }
