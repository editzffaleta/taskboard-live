...
- [ ] 1.1 `001-base-do-projeto` — base tecnica do monorepo (namespace `@taskboard`, Prisma,
...
- [ ] 1.3 `003-registro-usuario` — modulo `auth`, agregado `user` (name/email/password),
  `crypto.provider` (bcrypt), `register-user`, `POST /auth/register`, tela `/join`. **Pre:** `001`, `002`.
- [ ] 1.4 `004-login-sessao` — `login-user`, JWT `{ sub, name, email }`, `AuthContext`/`AuthGuard`,
...
- [ ] 2.1 `005-quadros-crud` — modulo `board`, agregados `board` + `membership`; CRUD de quadros,
  "meus quadros", owner nasce como membro; guard de acesso por `BoardMember`. **Pre:** `004`.
- [ ] 2.2 `006-tempo-real-gateway` — Socket.IO no NestJS: `BoardGateway`, handshake JWT, salas
  `board:{id}` com autorizacao por membro, presenca, e a porta `RealtimeEmitter`. **Pre:** `004`, `005`.
- [ ] 2.3 `007-listas-colunas` — agregado `list` (colunas): criar/renomear/excluir/reordenar;
  emite `list.*` via `RealtimeEmitter`. **Pre:** `005`, `006`.
- [ ] 2.4 `008-cartoes` — agregado `card`: criar/editar/excluir/mover (entre colunas + posicao);
  emite `card.*` (incl. `card.moved`) via `RealtimeEmitter`. **Pre:** `007`, `006`.
- [ ] 2.5 `009-quadro-ao-vivo-ui` — **vitrine**: pagina `/boards/[id]` com colunas/cartoes,
...
- [ ] 2.6 `010-compartilhamento-membros` — adicionar/remover membros por e-mail (so owner),
  listar; emite `member.added`; painel "Compartilhar" na UI. **Pre:** `005`, `006`, `009`.
- [ ] 2.7 `011-atividade-do-quadro` — agregado `activity` + `ActivityRecorder`; feed paginado
...
- [ ] 3.1 `012-hardening-http` — helmet, CORS explicito (HTTP + Socket.IO), limite de payload,
  rate limit global + estrito em `POST /auth/login` e `POST /auth/register`. **Pre:** `001`, `004`.
- [ ] 3.2 `013-fundacao-e2e` — Playwright + smoke de auth + **spec vitrine**: dois navegadores no
