...
export interface UnassignLabelIn {
  boardId: string;
  cardId: string;
  labelId: string;
  requesterId: string;
...
export interface UnassignLabelOut {
  card: Card;
  labels: Label[];
}

export class UnassignLabel
  implements UseCase<UnassignLabelIn, UnassignLabelOut>
{
  constructor(
  ...
  async execute(input: UnassignLabelIn): Promise<UnassignLabelOut> {
    Validator.validate([
      {
        code: "unassignLabel.boardId",
        value: input.boardId,
        rules: [new RequiredRule(), new UuidRule()],
      },
      {
        code: "unassignLabel.cardId",
        value: input.cardId,
        rules: [new RequiredRule(), new UuidRule()],
      },
      {
        code: "unassignLabel.labelId",
        value: input.labelId,
        rules: [new RequiredRule(), new UuidRule()],
      },
      {
        code: "unassignLabel.requesterId",
    ...
    await this.cardLabelRepository.unassign(card.id, label.id);
