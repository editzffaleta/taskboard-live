...
import { AssignUser } from "../../../src/card-assignee/usecase/assign-user.usecase";
import {
...
describe("AssignUser", () => {
  it("atribui um membro do quadro como responsavel", async () => {
    ...
    expect(assigneeIds).toEqual([OTHER_MEMBER_ID]);
  });

  it("e idempotente ao atribuir o mesmo responsavel duas vezes", async () => {
    ...
    expect(cardAssigneeRepository.links).toHaveLength(1);
    expect(assigneeIds).toEqual([OTHER_MEMBER_ID]);
  });

  it("rejeita usuario que nao e membro do quadro", async () => {
    const { useCase, card } = await setup();
