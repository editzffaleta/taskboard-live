...
import { DeleteChecklistItem } from "../../../src/checklist-item/usecase/delete-checklist-item.usecase";
import {
  FakeCardRepository,
...
describe("DeleteChecklistItem", () => {
  it("remove o item sem recalcular posicoes dos demais", async () => {
    const { useCase, checklistItemRepository, card, item } = await setup();
    await checklistItemRepository.create(
      new ChecklistItem({ cardId: card.id, text: "Item 2", position: 1 }),
    );
    ...
    expect(checklist[0].position).toBe(1);
  });
