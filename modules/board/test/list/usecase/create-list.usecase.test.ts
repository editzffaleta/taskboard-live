import { DomainError, ValidationException } from "@taskboard/shared";
import { Membership } from "../../../src/membership/model";
import { CreateList } from "../../../src/list/usecase/create-list.usecase";
import { FakeListRepository, FakeMembershipRepository } from "../../mock";

const BOARD_ID = "3fa85f64-5717-4562-b3fc-2c963f66afa6";
const MEMBER_ID = "9c858901-8a57-4791-81fe-4c455b099bc9";
const OTHER_USER_ID = "e22fa8be-9843-4769-930a-62d4f26e5e1e";

function setup() {
  const memberships: Membership[] = [
    new Membership({ boardId: BOARD_ID, userId: MEMBER_ID, role: "owner" }),
  ];
  const listRepository = new FakeListRepository();
  const membershipRepository = new FakeMembershipRepository(memberships);
  const useCase = new CreateList(listRepository, membershipRepository);
  return { listRepository, membershipRepository, useCase };
}

describe("CreateList", () => {
  it("cria a lista com position no fim das listas do quadro", async () => {
    const { listRepository, useCase } = setup();

    const { list: first } = await useCase.execute({
      boardId: BOARD_ID,
      requesterId: MEMBER_ID,
      title: "A fazer",
    });
    const { list: second } = await useCase.execute({
      boardId: BOARD_ID,
      requesterId: MEMBER_ID,
      title: "Em progresso",
    });

    expect(first.position).toBe(0);
    expect(second.position).toBe(1);
    expect(listRepository.lists).toHaveLength(2);
  });

  it("rejeita quando o requester nao e membro do quadro", async () => {
    const { useCase } = setup();

    await expect(
      useCase.execute({
        boardId: BOARD_ID,
        requesterId: OTHER_USER_ID,
        title: "A fazer",
      }),
    ).rejects.toMatchObject({
      message: "board.member.required",
      statusCode: 403,
    });
    await expect(
      useCase.execute({
        boardId: BOARD_ID,
        requesterId: OTHER_USER_ID,
        title: "A fazer",
      }),
    ).rejects.toBeInstanceOf(DomainError);
  });

  it("rejeita entrada invalida (titulo vazio)", async () => {
    const { useCase } = setup();

    await expect(
      useCase.execute({ boardId: BOARD_ID, requesterId: MEMBER_ID, title: "" }),
    ).rejects.toBeInstanceOf(ValidationException);
  });
});
