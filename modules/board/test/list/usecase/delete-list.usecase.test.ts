...
import { DeleteList } from "../../../src/list/usecase/delete-list.usecase";
...
function setup() {
  ...
  const useCase = new DeleteList(listRepository, membershipRepository);
  return { listRepository, membershipRepository, useCase };
}

describe("DeleteList", () => {
  it("permite o membro excluir a lista", async () => {
    ...
    expect(listRepository.lists).toHaveLength(0);
  });
  ...
  it("rejeita quando o requester nao e membro do quadro", async () => {
    ...
    await expect(
      useCase.execute({ listId: list.id, requesterId: OTHER_USER_ID }),
    ...
    expect(listRepository.lists).toHaveLength(1);
  });
