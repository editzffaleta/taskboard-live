...
- [ ] 1.1 Ler `apps/frontend/src/shared/navigation/app-navigation.config.ts`,
...
- [ ] 1.2 Reescrever `app-navigation.config.ts`: substituir `APP_NAVIGATION_SECTIONS`
...
- [ ] 1.3 Simplificar `sidebar-menu.component.tsx`: remover os ramos de `moduleNavigation`
...
- [ ] 2.1 Reescrever `app-sidebar-navigation.component.tsx` para renderizar, nesta ordem, a
...
- [ ] 2.2 Fazer o campo "Buscar…" abrir o `CommandPalette` (`023`) ao ser clicado: em
...
- [ ] 2.3 Fazer o item "Notificações" da sidebar abrir a central de notificações real (`024`):
...
- [ ] 2.4 Adicionar o prop opcional `triggerTestId?: string` (default
...
- [ ] 3.1 Ajustar `admin-shell.component.tsx` apenas no necessário para acomodar a nova sidebar
...
- [ ] 3.2 No menu do usuário (`DropdownMenu` de `admin-shell.component.tsx`), confirmar que o
...
- [ ] 3.3 Ajustar `apps/frontend/src/app/(private)/layout.tsx` para a nova assinatura de
...
- [ ] 4.1 Rodar `npx tsc --noEmit -p apps/frontend/tsconfig.json` sem erros.
  - **Aceite:** typecheck limpo.

- [ ] 4.2 Rodar `npx turbo run lint check-types --filter=@taskboard/frontend` sem novos
  erros/warnings introduzidos por esta change.
  - **Aceite:** lint limpo (ou só warnings pré-existentes, documentados).

- [ ] 4.3 Rodar `NEXT_IGNORE_INCORRECT_LOCKFILE=1 npm --workspace @taskboard/frontend run build`
  com sucesso.
  - **Aceite:** build verde; nenhuma rota removida do output esperado (`/boards`,
    `/boards/[id]`, `/templates`, `/archived`, `/account`, etc.).

- [ ] 4.4 Validar manualmente (ou por leitura de código, se backend não estiver disponível no
...
- [ ] 4.5 Confirmar zero lorem/placeholder e zero resquício do switcher de dois níveis:
...
- [ ] 4.6 Rodar `openspec validate 027-shell-navegacao --strict` e corrigir qualquer falha antes
